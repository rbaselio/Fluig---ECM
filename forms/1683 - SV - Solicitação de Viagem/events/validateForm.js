function validateForm(form){
	...
	if(getValue("WKCompletTask") == 'true' ){
		...
		if (numState == 139) {
			...
			if (!verifica_Hora(form.getValue("hora_retorno"))) message += "<br/>- Horario de Entrada;";
			
			var km_ini = parseFloat(form.getValue("km_ini").replace(".","").replace(",","."));
			if (isNaN(km_ini)){
				message += "<br/>- Km inicial;"
			}
			
			var km_fim = parseFloat(form.getValue("km_fim").replace(".","").replace(",","."));
			if (isNaN(km_fim)){
				message += "<br/>- Km Final;"
			}
			
			if (km_fim <= km_ini) message += "<br/>- A quilometragem incial não pode ser maior que a final;";
			...
			if (isNaN(datasaida.getTime()))  message += "<br/>- Data ou hora de saida;"; 
	
			var dt_retorno = form.getValue("dt_retorno").split("/");
			var hora_retorno = form.getValue("hora_retorno").split(":");		
			var dataretorno = new Date(dt_retorno[2], dt_retorno[1] - 1, dt_retorno[0], hora_retorno[0], hora_retorno[1], '00');
			if (isNaN(dataretorno.getTime()))  message += "<br/>- Data ou hora de retorno;"; 
	
			if (dataretorno <= datasaida) message += "<br/>- A data e hora de retorno não pode ser maior que a de saida;";
