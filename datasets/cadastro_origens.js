function createDataset(fields, constraints, sortFields) {
	...
	var datasetDoFormulario = "cadastro_de_origens";

	//nome da tabela do formulario
	var tablename = "tb_origem";
	...
	for (var i = 0; i < datasetPrincipal.rowsCount; i++) {
		...
		var datasetFilhos =  DatasetFactory.getDataset(datasetDoFormulario, null, constraintsFilhos, new Array("origem"));
		for (var j = 0; j < datasetFilhos.rowsCount; j++) {
			filhos[count] = {Origem: datasetFilhos.getValue(j, "origem")};
			count++;			
	...
	filhos.forEach(function(filho) {
		dataset.addRow( new Array(filho.Origem));
		log.info("================filho.Origem: " + filho.Origem);
	});	
