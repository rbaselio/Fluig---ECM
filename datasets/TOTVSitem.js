function createDataset(fields, constraints, sortFields) {
	...
	var cod_item = '01001001';
	var estab = '1';
	
	if (constraints != null) {
        for (var i = 0; i < constraints.length; i++) { 
        	if (constraints[i].fieldName == 'cod_item' ) cod_item = constraints[i].initialValue;
        	if (constraints[i].fieldName == 'estab' ) estab = constraints[i].initialValue;
        }
	}
	
	var newDataset = DatasetBuilder.newDataset();
	
	var QueryPrincipal = 'select "it-codigo", "desc-item", "cod-destaq" as NCM, "aliquota-ipi" from PUB.ITEM where "it-codigo" = \'' + cod_item + '\'';
	
	var QuerySUB = 'select "it-codigo", "item-estab"."val-unit-mat-m", "item-estab"."val-unit-mob-m", "item-estab"."val-unit-ggf-m" from PUB."item-estab" where "cod-estabel" = ' + estab + ' and "it-codigo" = \'' + cod_item + '\'';
	...
	try {
		...
		if (!created) {
			...
			if (rsMGMOV.getMetaData().getColumnCount() > 0){
				rsMGMOV.next();
				Arr[columnCount] = rsMGMOV.getObject("val-unit-mat-m").toString().split(';')[0];
				Arr[columnCount + 1] = rsMGMOV.getObject("val-unit-mob-m").toString().split(';')[0];
				Arr[columnCount + 2] = rsMGMOV.getObject("val-unit-ggf-m").toString().split(';')[0];
				
				log.warn(rsMGMOV.getObject("val-unit-mat-m").toString().split(';')[0]);
