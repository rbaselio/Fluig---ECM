function defineStructure() {
	addColumn("idade");
	addColumn("ativo");
	addColumn("aniversario");
	addColumn("narcimento");
	addColumn("nome");
	addColumn("matricula");
	setKey([ "matricula", "nome" ]);
	addIndex([ "matricula" ]);
	addIndex([ "matricula", "nome"]);
	...
	try{
	  ...
	  //campos da tabelas
	    ...
	    tt_funcionario.value = campos_tabela;
		 ...
		 var params = [tt_funcionario];
		
		//conversor dos parametros de input para Json
		var jsonParams = JSON.stringify(params);
    	
	    //log.warn(">>>>>>>>>>>>>>>>>>>>>>>>PASSO 5");	
	    var resp = service.callProcedureWithToken(token, "webservices/esws0003.r", "getColaboradores", jsonParams);
