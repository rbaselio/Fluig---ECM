...
function ativaPreencheCampos(modeView, numState, WKNumProces, documentId) {
	...
	if (modeView == "ADD" || modeView == "MOD") {
		...
		if (numState == 11) {
			...
			$('#num_processo').val(WKNumProces);
			if ($('#data_resp').val() == "") $('#data_resp').val(data);
			$('#data_resp_ultima').val(data)
		...
		if (numState == 9) {
			getUsuario.done(function(response) {
				$('#matricula_qualidade').val(response.content.userCode);
				$('#user_qualidade').val(response.content.name);
						
			});
			$('#data_qualidade').val(data);
			showElemento($("#div_qualidade"));		
		}
	}
}
