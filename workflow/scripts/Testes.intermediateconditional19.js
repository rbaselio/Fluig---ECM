function intermediateconditional19() {
		...
		for ( var key in keys) {
			...
			if (field.indexOf("processo___") > -1) {
				...
				if (datasetPrincipal.rowsCount > 0) {
					...
					if (hAPI.getCardValue("status___" + row) != "Cancelado"){
						hAPI.setCardValue("status___" + row, "Concluído");
					}
					log.warn("O Processo " + processo + " esta encerrado / cancelado");
				}
			}
		}
