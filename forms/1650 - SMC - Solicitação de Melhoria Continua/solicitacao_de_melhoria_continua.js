...
$(function(ready){
	...
	$('#tipo_outros').change(function() {
		if($(this).is(':checked')) $('#det_outros').removeAttr('disabled');
		else {
...
var beforeSendValidate = function(numState){
	...
	if (numState == 0 || numState == 1){
		...
		var tipos = false;		
		$("input:checkbox[id^='tipo_']").each(function(i) {
			if ($(this).is(':checked')) tipos = $(this).is(':checked');
		});
		if (!tipos) message += "<br/>Tipo de Melhoria";			
	}	
	...
	if (numState == 16){		
		...
		$("input[id^='tipo_ganho']").each(function(i) {
			if ($(this).closest('tr').attr('style') != "display:none" && $(this).val() == ""){
				message += "Tipo de ganho na linha" + $(this).closest('tr').index();
			}
		});	
